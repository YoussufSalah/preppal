...
const ModernFlashcards = ({ flashcardData = {}, isLoading = false }) => {
    ...
    if (
        ...
        !currentFlashcardData.cards ||
        currentFlashcardData.cards.length === 0 ||
        !flashcardData
    ) {
        return (
            <div className="w-full min-w-0 bg-white/80 backdrop-blur-sm rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg">
                <div className="flex items-center justify-center h-64 sm:h-80">
                    <div className="text-center space-y-4">
                        ...
                        <p className="text-sm sm:text-base text-gray-600 px-4">
                            No flashcards available. Upload a document to
                            generate flashcards.
                        </p>
                    </div>
                </div>
    ...
    return (
        <div
            className="w-full min-w-0 bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-300 animate-fade-in"
        ...
        >
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6 lg:mb-8">
                ...
                <span className="bg-purple-100 text-purple-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium animate-pulse">
                    {currentCardIndex + 1} /{" "}
                    {currentFlashcardData.count ||
                        currentFlashcardData.cards.length}
                </span>
            </div>
                    ...
                    return (
                        <div
                            ...
                            style={{
                                transform: `translateX(${
                                    offset * 15 + (isActive ? dragOffset : 0)
                                }px) scale(${
                                    isActive
                                        ? 1
                                        : 0.95 - Math.abs(offset) * 0.05
                                })`,
                                opacity:
                                    Math.abs(offset) > 1
                                        ? 0
                        ...
                        >
                            <div
                                ...
                                onMouseDown={
                                    isActive ? handleMouseDown : undefined
                                }
                                onMouseMove={
                                    isActive ? handleMouseMove : undefined
                                }
                                onMouseUp={isActive ? handleMouseUp : undefined}
                                onMouseLeave={
                                    isActive ? handleMouseUp : undefined
                                }
                                onTouchStart={
                                    isActive ? handleTouchStart : undefined
                                }
                                onTouchMove={
                                    isActive ? handleTouchMove : undefined
                                }
                                onTouchEnd={
                                    isActive ? handleTouchEnd : undefined
                                }
                            >
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6">
                ...
                <button
                    ...
                    disabled={
                        currentCardIndex ===
                        currentFlashcardData.cards.length - 1
                    }
                    className={`flex items-center px-3 sm:px-4 py-2 sm:py-3 rounded-lg font-medium text-sm sm:text-base transition-all touch-manipulation ${
                        currentCardIndex ===
                        currentFlashcardData.cards.length - 1
            ...
            <div className="flex justify-center">
                <button
                    onClick={() =>
                        downloadAnkiCSV(flashcardData, "PrepPal-Flashcards.csv")
                    }
